...
// Verificar autenticación y obtener reportes de campañas SMS
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  // Obtener reportes de campañas SMS
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    // Validar formato del CSV (debe tener solo una columna)
    if (lines.length < 2) {
    ...
    // Verificar que todas las líneas tengan solo una columna
    for (let i = 1; i < lines.length; i++) {
      const columns = lines[i].split(',');
      if (columns.length > 1) {
        console.error(`❌ Línea ${i + 1} tiene ${columns.length} columnas`);
        return {
          error: `Línea ${i + 1}: El archivo debe tener solo una columna (números de teléfono). Se encontraron ${columns.length} columnas.`
        };
      }
    }
    ...
    // Insertar reporte en sms_campaign_report
    ...
    const { data: reportData, error: reportError } = await supabaseAdmin
      ...
      .insert({
        ...
        description: `Campaña SMS - ${campaignNumber}`,
    ...
    // Procesar cada fila del CSV y insertar en sms_campaign
    const csvLines = lines.slice(1); // Excluir header
    console.log("📱 Procesando", csvLines.length, "números de teléfono");
    ...
    const campaignRecords = csvLines.map((line, index) => {
      ...
      if (!phoneNumber) {
        throw new Error(`Línea ${index + 2}: Número de teléfono vacío`);
      }
    ...
    console.log("💾 Iniciando inserción masiva en sms_campaign...");
    ...
    // Actualizar el conteo real en sms_campaign_report
    ...
    return {
      success: `Campaña ${campaignNumber} procesada exitosamente. ${totalInserted} números de teléfono guardados.`
    };
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Campañas SMS</h1>
              <p className="text-gray-600">Administra y supervisa las campañas de mensajes SMS</p>
            </div>
          </div>
          <div className="flex space-x-3">
            ...
            <button 
              onClick={() => {
                ...
                link.setAttribute('download', `campañas_sms_${new Date().toISOString().split('T')[0]}.csv`);
                  ...
                  <tr key={report.campaign_number} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        <Link
                          to={`/campanas/sms/${report.campaign_number}`}
